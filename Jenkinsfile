...
pipeline {
 ...
 stages {
        stage('Checkout-git'){
               steps{
		git poll: true, url: 'git@github.com:cgaldea/test-jenkins-1.git'
               }
        }
    ...
    stage('PushDockerImage') {
            steps {
            	sh '''
            		docker tag apptest:latest carlosdockerw10/apptest:latest
					docker push carlosdockerw10/apptest:latest
